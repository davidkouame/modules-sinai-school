    ...
    <div class="row card-body-header">
      ...
      <div class="col-sm-6">
        <div class="float-right">
          <div class="row">
            <div class="col-md-8">
              <input type="text" class="form-control search" placeholder="Rechercher une section" @keyup="searchModel" v-model="search">
            </div>
            <div class="col-md-4">
              <a :href="'#/sections-annee-scolaire/add/'" class="btn btn-primary btn-add"  v-if="checkPermission('ADD_SECTIONANNEESCOLAIRE')">Ajouter</a>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="col-sm-12 card-body-body">
      <table class="table table-d">
        ...
        <tbody>
          <tr v-if="sectionsanneescolaire" v-for="(sectionanneescolaire, index) in sectionsanneescolaire">
            ...
            <td>
              ...
              <a :href="'#/sections-annee-scolaire/edit/'+sectionanneescolaire.id"  class="btn btn-icon btn-success btn-sm" v-if="checkPermission('EDIT_SECTIONANNEESCOLAIRE')">
                <!----><i class="fa fa-edit"></i><!---->
              </a>&nbsp;
              <a id="show-modal" @click="showModalF(sectionanneescolaire.id)" type="button" class="btn btn-icon btn-danger btn-sm btn-delete" v-if="checkPermission('DELETE_SECTIONANNEESCOLAIRE')">
                <!----><i class="fa fa-trash"></i><!---->
              </a>
            </td>
          </tr>
          <tr v-show="showEmptySentenceSectionAnneeScolaire"><td colspan="6" style="text-align: center;">Aucun resultat trouvé !</td></tr>
        </tbody>
      </table>
        ...
        <modal
                  ...
                  v-bind:modelid="sectionAnneeScolaireId"
                  modelname="sectionanneescolaire"
                  textBody="Voulez vous supprimez cette section ?"
                  nameUrl="sectionsanneescolaire"
                ></modal>
                  ...
                  <div class="col-md-8">
                    <div class="float-right pagi">
                      <paginate
                        ...
                        :click-handler="dispatchSectionsAnnneeScolaire"
                        :prev-text="'&laquo;'"
                        :next-text="'&raquo;'"
...
export default {
  ...
  created(){
    this.dispatchSectionsAnnneeScolaire();
  },
  methods: {
    dispatchSectionsAnnneeScolaire(pageNum, search = null){
      pageNum = pageNum == null ? 1 : pageNum
      let params = [
        {key: 'search', value: search}, 
        {key: 'school_id', value: this.$cookies.get('ecoleId')},
        {key: 'annee_scolaire_id', value: this.$cookies.get('anneeScolaireId')}];
      this.$store.dispatch('getSectionsAnneeScolaire', 
    ...
    showModalF(sectionAnneeScolaireId = null) {
      this.showModal = true;
      this.sectionAnneeScolaireId = sectionAnneeScolaireId;
    },
    searchModel(){
      this.dispatchSectionsAnnneeScolaire(1, this.search)
    }
  },
  computed:{
