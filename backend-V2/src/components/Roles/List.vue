  ...
  <card class="card" :title="title">
      ...
      <div class="col-sm-6">
        <div class="float-right">
          <div class="row">
            ...
            <div class="col-md-4">
              <a :href="'#/roles/add/'" class="btn btn-primary btn-add" v-if="checkPermission('ADD_ROLE')">Ajouter</a>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="col-sm-12 card-body-body">
      <table class="table table-d">
        ...
        <tbody>
          <tr v-if="roles" v-for="(role, index) in roles">
            <th scope="row">{{ indexPagnation + index + 1}}</th>
            <td>{{ role.libelle}}</td>
            <td>
              <a :href="'#/roles/preview/'+role.id" class="btn btn-icon btn-info btn-sm">
                <!---->
                <i class="fa fa-eye"></i>
                <!---->
              </a>&nbsp;
              <a :href="'#/roles/edit/'+role.id" class="btn btn-icon btn-success btn-sm" v-if="checkPermission('EDIT_ROLE')">
                <!---->
              ...
              <a
                id="show-modal"
                @click="showModalF(role.id)"
                type="button"
                class="btn btn-icon btn-danger btn-sm btn-delete" v-if="checkPermission('DELETE_ROLE')"
              >
                <!---->
          ...
          <tr v-show="showEmptySentenceRole">
      ...
      <modal
        ...
        v-bind:modelid="roleId"
        modelname="role"
        textBody="Voulez vous supprimez cet rôle ?"
        nameUrl="roles"
      ></modal>
...
export default {
  ...
  methods: {
    dispatchRole(pageNum, search = null) {
    ...
    showModalF(roleId = null) {
      this.showModal = true;
      this.roleId = roleId;
    },
    searchModel() {
      this.dispatchRole(1, this.search);
    }
  },
  computed: {
    roles() {
    ...
    showEmptySentenceRole() {
      return this.notEmptyObject(this.roles) == 0;
    }
  },
  watch: {
    currentPage() {
