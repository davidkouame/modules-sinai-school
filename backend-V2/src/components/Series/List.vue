    ...
    <div class="row card-body-header">
      ...
      <div class="col-sm-6">
        <div class="float-right">
          <div class="row">
            <div class="col-md-8">
              <input type="text" class="form-control search" placeholder="Rechercher une serie" @keyup="searchModel" v-model="search">
            </div>
            <div class="col-md-4">
              <a :href="'#/series/add/'" class="btn btn-primary btn-add">Ajouter</a>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="col-sm-12 card-body-body">
      <table class="table table-d">
        ...
        <tbody>
          <tr v-if="series" v-for="(serie, index) in series">
            ...
            <td>
              ...
              <a id="show-modal" @click="showModalF(serie.id)" type="button" class="btn btn-icon btn-danger btn-sm btn-delete">
                <!----><i class="fa fa-trash"></i><!---->
              </a>
            </td>
          </tr>
          <tr v-show="showEmptySentenceSerie"><td colspan="6" style="text-align: center;">Aucun resultat trouvé !</td></tr>
        </tbody>
      </table>
        ...
        <modal
                  ...
                  v-bind:modelid="serieId"
                  modelname="serie"
                  textBody="Voulez vous supprimez cette serie ?"
                  nameUrl="series"
                ></modal>
...
export default {
  ...
  methods: {
    dispatchSeries(pageNum, search = null){
    ...
    showModalF(serieId = null) {
      this.showModal = true;
      this.serieId = serieId;
    },
    searchModel(){
      this.dispatchSeries(1, this.search)
    }
  },
  computed:{
    series(){
